...
import { getOrCreateAppUser, type AppUser as AppUserWithRoles } from "@/lib/auth/CurrentUser";
 ...
 */
export async function isAdmin(user?: AppUserWithRoles): Promise<boolean> {
  try {
    ...
    const userToCheck = user || await getOrCreateAppUser();
    if (!userToCheck) {
      return false;
    }

    // Check database roles
    if (userToCheck.roles && Array.isArray(userToCheck.roles) && userToCheck.roles.includes("ADMIN")) {
      return true;
    }
    ...
    if (adminUserIds.length > 0 && userToCheck.id) {
      const isAdminById = 
        adminUserIds.includes(userToCheck.id) ||
        (userToCheck.clerkId && adminUserIds.includes(userToCheck.clerkId));
 ...
 */
export async function requireAdmin(): Promise<AppUserWithRoles> {
